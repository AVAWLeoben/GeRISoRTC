...
class MvImpactNIRCamera:
    ...
    def _prepare_classifier(self):
        if self.input_mode not in ("classified", "classified_line", "classified_line_312x1", "smart"):
    ...
    def _apply_basic_settings(self):
        ...
        try:
            ac = acquire.AcquisitionControl(self.device)
            if self.fps > 0:
                for enable_name in ("acquisitionFrameRateEnable", "mvAcquisitionFrameRateEnable"):
                    self._write_property(ac, enable_name, True)
                for fps_name in ("acquisitionFrameRate", "mvAcquisitionFrameRate"):
                    self._write_property(ac, fps_name, self.fps)
        except Exception as exc:
            print(f"[mvImpact NIR] AcquisitionControl not fully available: {exc}")
    ...
    def _pace_fps(self):
        ...
        now = time.monotonic()
        wait = min_interval - (now - self._last_frame_time)
        if wait > 0:
            time.sleep(wait)
    ...
    def _process_native_sample(self, native: np.ndarray) -> np.ndarray:
        ...
        self.last_line_timestamp = time.monotonic()
    ...
    def _classify_spectral_sample(self, spectral: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            print(f"[mvImpact NIR] Classifier predict failed: {exc}")
            print("[mvImpact NIR] Switching to PlaceholderSAMClassifier.")
            self._classifier = PlaceholderSAMClassifier(
                n_classes=self.synthetic_classes,
                background_threshold=self.background_threshold,
            )
            self._classifier_name = "PlaceholderSAMClassifier"
            return self._classifier.predict(spectral).astype(np.uint8) % max(1, self.synthetic_classes)
